...
def trip_distribution(trajs: list, n_grid: int, _epsilon: float, level_list: list) -> np.ndarray:
    """
    ...
    for i in range(n_grid):
        ...
        for j in range(n_grid):
            used_rate = 1 - (max(level_list[i], level_list[j])) / sum_of_level
            noise = np.random.laplace(0, 1 / _epsilon * used_rate)  # add laplacian noise
            R[i][j] += noise
...
def trip_distribution_main(n_grid: int, _epsilon: float, grid_trajs_path: str,
                           trip_distribution_path: str, level_list: list):
    """
    ...
    with open(grid_trajs_path, 'r') as grid_trajs_file:
        ...
        with open(trip_distribution_path, 'w') as trip_distribution_file:
            trip_distribution_matrix = trip_distribution(T, n_grid, _epsilon, level_list)
