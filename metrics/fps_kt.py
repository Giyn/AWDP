    ...
    for file in trajs_file_list:
        with open(trajs_path + file, 'r') as traj_file:
            ...
            for line in traj_file.readlines():
                ...
                if len(grid_num_list) > 0 and lat_to_grid * grid_side + lon_to_grid == \
                        grid_num_list[-1]:
                    continue
...
def cal_fps(raw_trajs_frequent_pattern: dict, sd_trajs_frequent_pattern: dict) -> float:
    ...
    for p in list(raw_trajs_frequent_pattern.keys())[:50]:
        if p in sd_trajs_frequent_pattern.keys():
            re = abs(raw_trajs_frequent_pattern[p] - sd_trajs_frequent_pattern[p]) / \
                 raw_trajs_frequent_pattern[p]
            FP += re
        ...
        if list(raw_trajs_frequent_pattern.keys())[i] in sd_trajs_frequent_pattern.keys():
            for j in range(i + 1, len(list(raw_trajs_frequent_pattern.keys())[:k])):
                if list(raw_trajs_frequent_pattern.keys())[j] in sd_trajs_frequent_pattern.keys():
                    if (raw_trajs_frequent_pattern[list(raw_trajs_frequent_pattern.keys())[i]] >=
                        raw_trajs_frequent_pattern[list(raw_trajs_frequent_pattern.keys())[j]]
                        and sd_trajs_frequent_pattern[list(raw_trajs_frequent_pattern.keys())[i]] >
                        sd_trajs_frequent_pattern[list(raw_trajs_frequent_pattern.keys())[j]]) \
                            or (raw_trajs_frequent_pattern[
                                    list(raw_trajs_frequent_pattern.keys())[i]] <
                                raw_trajs_frequent_pattern[
                                    list(raw_trajs_frequent_pattern.keys())[j]]
                                and sd_trajs_frequent_pattern[
                                    list(raw_trajs_frequent_pattern.keys())[i]] <
                                sd_trajs_frequent_pattern[
                                    list(raw_trajs_frequent_pattern.keys())[j]]):
                        concordant_count += 1
                    else:
                        discordant_count += 1
